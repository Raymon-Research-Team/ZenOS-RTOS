 ...
 * @brief   Internal shared declarations between ZenOS translation units.
 */
...
inline bool os_verify_period_gate_after_wake(TCB* task, uint32_t current_tick) {
    if (!task) return true;
    int32_t elapsed = (int32_t)(current_tick - task->next_run_time);
    return elapsed >= 0;
}
#endif
...
/* ZenOS is currently a single-core kernel. On that architecture an interrupt
   mask is the correct atomic primitive and is also available on Armv8-M.
   Using LDREX/STREX here made the kernel non-portable to Armv6-M and was
   unnecessary for a uniprocessor scheduler. */
inline void os_atomic_inc(volatile uint32_t* value) {
    uint32_t cs = os_critical_enter();
    ++(*value);
    os_critical_exit(cs);
}
inline void os_atomic_dec_saturating(volatile uint32_t* value) {
    uint32_t cs = os_critical_enter();
    if (*value != 0U) --(*value);
    os_critical_exit(cs);
}

/* Called by the architecture-specific PendSV implementation. */
