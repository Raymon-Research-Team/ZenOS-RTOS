...
/** Byte-access to PendSV and SysTick priority (SHPR3 register) */
...
/* ============================================================================
 *  Effective MPU availability
 *  ---------------------------------------------------------------------------
 *  Cortex-M3 does not imply an MPU. CMSIS exposes the actual device feature
 *  through __MPU_PRESENT. STM32F103xB defines it as 0, so a configuration that
 *  requests MPU safety must not dereference the nonexistent MPU registers.
 *  Keep the user-facing OS_SAFETY_MPU setting intact, but automatically turn
 *  the implementation off when the target explicitly reports no MPU.
 * ============================================================================ */
